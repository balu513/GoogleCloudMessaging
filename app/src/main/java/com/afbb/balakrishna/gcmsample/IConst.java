package com.afbb.balakrishna.gcmsample;

public class IConst {

    public static final String SENDER_ID = "457179918893";
    public static final String URL_REGISTRATION = "http://192.168.50.118/workspace/GcmApp/register.php";
    public static final String URL_SEND_MESSAGE = "http://192.168.50.118/workspace/GcmApp/send_message.php";

    public static final String PARAM_NAME = "name";
    public static final String PARAM_EMAIL = "email";
    public static final String PARAM_PHONE_NUMBER = "phno";
    public static final String PARAM_REGISTRATION_ID = "regId";
}
